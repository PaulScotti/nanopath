...
HARD_CLUSTERS, HARD_ITERATIONS, HARD_FRACTION = 8, 5, 0.5
...
# Online k-means over the EMA teacher's per-tile CLS features. Farthest-first initialization,
# lowest-index tie resolution, and retained empty centroids make selection reproducible and bounded.
def hardest_tile_indices(x):
    x = F.normalize(x.detach().float(), dim=-1)
    n, k = x.shape[0], min(HARD_CLUSTERS, x.shape[0])
    chosen = torch.zeros(n, dtype=torch.bool, device=x.device)
    centroid_idx = torch.empty(k, dtype=torch.long, device=x.device)
    centroid_idx[0] = 0
    chosen[0] = True
    for j in range(1, k):
        nearest = (x[:, None] - x[centroid_idx[:j]][None]).square().sum(-1).amin(1)
        nearest[chosen] = -1
        centroid_idx[j] = nearest.argmax()
        chosen[centroid_idx[j]] = True
    centroids = x[centroid_idx]
    for _ in range(HARD_ITERATIONS):
        assignment = (x[:, None] - centroids[None]).square().sum(-1).argmin(1)
        members = assignment[:, None] == torch.arange(k, device=x.device)
        counts = members.sum(0).unsqueeze(1)
        updated = torch.where(counts > 0, (members.T[:, :, None] * x).sum(1) / counts.clamp(min=1), centroids)
        centroids = updated
    distance = (x - centroids[(x[:, None] - centroids[None]).square().sum(-1).argmin(1)]).square().sum(-1)
    keep = min(n, max(2, math.ceil(n * HARD_FRACTION)))
    return distance.argsort(descending=True, stable=True)[:keep], distance
...
def main():
    ...
    train_cfg = cfg["train"]
    dino_cfg = cfg["dino"]
    # Optional early backbone freeze remains part of the inherited optimizer recipe.
    ...
    # The full EMA-teacher pass supplies stable representations for online selection; student forwards and every
    # optimized loss then use only the hardest half. Validation keeps the full batch for comparable diagnostics.
    def compute_losses(gf, lf, b, masks, t_temp, k_scale, ckpt=False, select_hardest=False):
        with torch.no_grad():
            t = teacher_backbone(gf)
            keep, distances = hardest_tile_indices(t["x_norm_clstoken"].view(train_cfg["global_views"], b, -1).mean(0)) if select_hardest else (torch.arange(b, device=device), None)
            gi = (torch.arange(train_cfg["global_views"], device=device)[:, None] * b + keep).flatten()
            li = (torch.arange(train_cfg["local_views"], device=device)[:, None] * b + keep).flatten()
            t_cls = teacher_dino_head(t["x_norm_clstoken"][gi]).chunk(train_cfg["global_views"])
            t_prob = sinkhorn(torch.cat((t_cls[1], t_cls[0])), t_temp).view(2, keep.numel(), -1)
        masks = masks[gi]
        mask_idx = masks.flatten().nonzero().flatten()
        mask_w = (1 / masks.sum(-1).clamp(min=1)).unsqueeze(-1).expand_as(masks)[masks]
        sg = student_backbone(gf[gi], masks=masks, checkpoint=ckpt)
        sl = student_backbone(lf[li], checkpoint=ckpt)
        ...
        target = F.layer_norm(t["x_norm_patchtokens"][gi].flatten(0, 1), (student_backbone.embed_dim,))[mask_idx]
        pred = student_predictor(sg["x_norm_patchtokens"]).flatten(0, 1)[mask_idx]
        jepa_loss = F.smooth_l1_loss(pred, target, reduction="none").mean(-1).mul(mask_w).sum() / gi.numel()
        ...
        return local_loss + global_loss, jepa_loss, kde, None if distances is None else distances[keep]
    ...
    def evaluate(eval_step, eval_teacher_temp, eval_kde_scale):
        ...
        for vb_idx, vbatch in enumerate(val_loader):
            ...
            with torch.no_grad(), autocast:
                ...
                masks = make_block_mask(b * train_cfg["global_views"], global_grid, device, n_blocks=int(dino_cfg["jepa_blocks"]), block_scale=float(dino_cfg["jepa_block_scale"]))
                dino_l, jepa_l, kde_v, _ = compute_losses(gf, lf, b, masks, eval_teacher_temp, eval_kde_scale)
    ...
    while examples_seen + batch_size <= max_train_samples and train_flops < max_train_flops:
        for batch in train_loader:
            ...
            # schedules to SAMPLE progress so they complete over the actual 1M-tile run.
            ...
            masks = make_block_mask(batch_size * train_cfg["global_views"], global_grid, device, n_blocks=int(dino_cfg["jepa_blocks"]), block_scale=float(dino_cfg["jepa_block_scale"]))
            ...
            with flop_ctx:
                with autocast:
                    ...
                    dino_loss_value, jepa_loss, kde, hardest_distances = compute_losses(
                        gf, lf, batch_size, masks, teacher_temp, kde_scale,
                        ckpt=activation_checkpointing, select_hardest=True,
                    )
                    total_loss = dino_loss_value + jepa_loss + kde
                opt.zero_grad(set_to_none=True)
                total_loss.backward()
                if examples_seen / max_train_samples < freeze_backbone_frac:  # Phase 1: backbone frozen; patch_embed and heads still train.
            ...
            visible_now = batch_size * train_cfg["global_views"] * global_patches + hardest_distances.numel() * train_cfg["local_views"] * local_patches
            ...
            if should_log:
                ...
                train_log = {
                    ...
                    **reduced,
                    ...
                    "hardest_distance_mean": float(hardest_distances.mean()),
                    "hardest_distance_min": float(hardest_distances.min()),
                    "selected_batch_size": hardest_distances.numel(),
                }
    ...
    summary = {
        ...
        **final_unique_counts,
        ...
        "hardest_tile_clusters": HARD_CLUSTERS,
        "hardest_tile_iterations": HARD_ITERATIONS,
        "hardest_tile_fraction": HARD_FRACTION,
